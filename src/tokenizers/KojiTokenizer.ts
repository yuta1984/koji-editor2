...
export default class KojiTokenizer extends Tokenizer {
    ...
    readID(): Token {
        ...
        if (this.state.inInlinePre || this.state.inBlockTag) {
            ...
            return { type: 'UNKNOWN', value: ch + id, error: true, errorMsg: 'ここにIDは書けません' }
        }
    }
    ...
    readClass(): Token {
        ...
        if (this.state.inInlinePre || this.state.inBlockTag) {
            ...
            return { type: 'UNKNOWN', value: ch + id, error: true, errorMsg: 'ここにクラスは書けません' }
        }
    }
    ...
    readNext(): Token | null {
        ...
        if (ch === '＃') tok = this.readID();
        if (ch === '＊') tok = this.readClass();
