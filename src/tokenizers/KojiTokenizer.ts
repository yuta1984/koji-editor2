...
export default class KojiTokenizer extends Tokenizer {
  ...
  readSymbol(): Token {
    ...
    if (ch === '□') {
      ...
      return {
        type: 'KAERITEN', value: ch + kaetiten
      };
    } else if (ch === '￣') {
      const okurigana = this.readWhile(ch => this.isKana(ch));
      return {
        type: 'OKURIGANA', value: ch + okurigana
      };
    }
    else {
  ...
  readNext(): Token | null {
    ...
    if (ch === '《' || ch === '＜') tok = this.readInlineStart();
    if (ch === '》' || ch === '＞') tok = this.readInlineEnd();
